...
app.get("/historial", function(request, response)
{
    ...
    let sql = "SELECT * FROM IRATEAMS.evento AS ev JOIN apuntados AS ap ON (ev.id_evento = ap.id_evento) JOIN usuario AS us ON (ap.id_usuario = us.id_usuario) WHERE us.id_usuario = ? AND  fecha < CURDATE();" 
...
app.post("/apuntados", function(request, response)
{
    ...
    let sql = `INSERT INTO apuntados(id_usuario, id_evento) 
